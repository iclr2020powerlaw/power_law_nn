...
# In[]
if reload:
    hiddenEigVals = np.load('models/Eig.npy', allow_pickle=True)[()]['eig']
else:
    ...
    for idx in range(len(hidden)):
        ...
        np.save('models/Eig', eigDict, allow_pickle=True)
...
fig.tight_layout()
fig.show()
fig.savefig('figures/mlp.pdf', transparency=True)
...
fig.tight_layout()
fig.show()
fig.savefig('figures/mlp_hist.pdf')